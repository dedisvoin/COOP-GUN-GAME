...
def send_packet_forming():
    ...
    send_packet.append(Packet([[player.collider.center, player.id, player.used_gun, int(player.player_gun_angle), player.gun_mir] for player in players], 'player_pos').get())
                ...
                if str(player.id) == str(id):
                    ...
                    if shoot == True:
                        ...
                        if data[1][1][3] and data[1][1][4]:
                            ...
                            create_bullet(data[1][1][1], player.collider.center, speed)
                
        except:...


def create_bullet(gun_name, pos, speed):
    global all_bullets
    
    all_bullets.append([
        gun_name, Vec2(*pos), speed, 0
    ])

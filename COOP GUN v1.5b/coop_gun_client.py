...
class Bullet:
    def __init__(self, sprite, name) -> None:
        self.sprite = Sprite.load_sprite(sprite)
        self.name = name

    def render(self, pos, speed):
        self.sprite.angle = Vec2(*speed).angle
        self.sprite.pos = pos
        self.sprite.render(app.surf)
...
bullets = [
    Bullet('aseprite\guns\standart_gun\standart_gun_bullet.png', 'Start Gun')
]
...
def main(delta):
    ...
    for i, pos in enumerate(person_poses):
        try:
            shapes.line(app.surf, [pos[0][0]+camera.pos[0]-100 / 10,pos[0][1]+camera.pos[1]-10], [pos[0][0]+camera.pos[0]+100/10,pos[0][1]+camera.pos[1]-10], 3, (255,200,200))
            shapes.line(app.surf, [pos[0][0]+camera.pos[0]-100 / 10,pos[0][1]+camera.pos[1]-10], [pos[0][0]+camera.pos[0]-100 / 10+pos[5]/5,pos[0][1]+camera.pos[1]-10], 3, 'red')
